...
# ==========================================
# Multimodal & Vision Schemas
# ==========================================
class MultimodalImageAnalysisRequest(BaseModel):
    image_base64: str = Field(..., description="Base64-encoded image data")
    filename: Optional[str] = Field("inventory_upload.jpg", description="Original image filename")
    mime_type: Optional[str] = Field("image/jpeg", description="MIME type: image/jpeg, image/png, image/webp")
    prompt: Optional[str] = Field(None, description="Optional frontline note or inspection instruction")

class MultimodalAnalysisResponse(BaseModel):
    filename: str
    detected_medicine: Optional[str] = None
    detected_medicine_code: Optional[str] = None
    ...
    confidence_score: float
    analysis_notes: str
    is_nlem_matched: bool
    requires_human_approval: bool = True
    advisory_disclaimer: str = (
        "Visual analysis generated by Gemini Multimodal. "
        "Advisory only — requires physical verification and human confirmation before recording inventory changes."
    )
    suggested_intake_prompt: Optional[str] = None
